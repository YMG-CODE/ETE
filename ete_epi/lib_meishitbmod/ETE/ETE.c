...
void ETE_oled_render_ballinfo(void) {
    ...
    oled_write(format_4d(ETE.last_mouse.x), false);
    oled_write(format_4d(-ETE.last_mouse.y), false);
    oled_write(format_4d(ETE.last_mouse.h), false);
    oled_write(format_4d(-ETE.last_mouse.v), false);
    // CPI
    oled_write_P(PSTR("     CPI"), false);
    oled_write(format_4d(ETE_get_cpi()) + 1, false);
    oled_write_P(PSTR("00  S"), false);
    oled_write_char(ETE.scroll_mode ? '1' : '0', false);
    oled_write_P(PSTR("  D"), false);
    oled_write_char('0' + ETE_get_scroll_div(), false);
#endif
}
...
void ETE_oled_render_keyinfo(void) {
    ...
    //
    // It is aligned to fit with output of ETE_oled_render_ballinfo().
    // For example:
    //
    //     Key    R2  C3 K06  'c
    //
    uint8_t keycode = ETE.last_kc;
